    ...
    <>
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="max-w-[95vw] md:max-w-[1200px] h-[90vh] md:h-[85vh] p-0 gap-0 overflow-hidden flex flex-col rounded-md border-border shadow-2xl bg-background">
          ...
          <div className="flex-1 overflow-y-auto md:overflow-hidden grid grid-cols-1 md:grid-cols-[1fr_380px]">

            {/* Items Column */}
            <div className={cn("h-auto md:h-full md:overflow-y-auto bg-background p-0 scrollbar-thin", items.length === 0 && "md:col-span-2")}>
              {items.length === 0 ? (
                ...
                <div className="divide-y divide-border/60">
                  {items.map((item) => (
                    <div key={`${item.productId}-${item.variantId}`} className="p-4 sm:p-6 flex flex-row gap-3 sm:gap-6 hover:bg-muted/5 transition-colors">
                      {/* Product Image - Responsive Size */}
                      <div className="w-20 h-20 sm:w-40 sm:h-40 bg-white border border-border/60 rounded-sm flex items-center justify-center shrink-0 p-2 sm:p-4 overflow-hidden relative">
                        {item.image ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-muted/10">
                            <div className="w-6 h-6 sm:w-12 sm:h-12 rounded-full" style={{ backgroundColor: item.colorCode || '#ddd' }} />
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div className="flex justify-between items-start gap-2 sm:gap-4">
                          <div className="space-y-1">
                            <h4 className="font-bold text-sm sm:text-xl text-foreground leading-tight line-clamp-2">{item.productName}</h4>
                            <div className="flex flex-wrap items-center gap-1.5 sm:gap-3">
                              <span className="text-[9px] sm:text-xs font-mono text-muted-foreground bg-muted/30 px-1 sm:px-1.5 py-0.5 rounded-sm border border-border/50 uppercase tracking-widest">
                                {item.modelNumber}
                              </span>
                              <span className="text-[10px] sm:text-sm font-medium text-foreground/80 line-clamp-1">
                                {item.variantName}
                              </span>
                              {item.colorCode && (
                                <div className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 rounded-full border border-border/50 shadow-sm" style={{ backgroundColor: item.colorCode }} />
                              )}
                            </div>
                          </div>
                        ...
                        <div className="mt-2 sm:mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0">
                          {/* Mobile Price Display - Now above quantity on mobile */}
                          ...
                          <div className="flex items-center justify-between sm:justify-start w-full sm:w-auto">
                            ...
                            <div className="flex items-center gap-3 sm:gap-6">
                              <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center border rounded-sm bg-background h-7 sm:h-9 shadow-sm">
                                  <button
                                    className="h-full w-7 sm:w-9 flex items-center justify-center border-r hover:bg-muted disabled:opacity-50 transition-colors cursor-pointer"
                                  ...
                                  <Input
                                    ...
                                    className="w-10 sm:w-12 h-full border-none text-center focus-visible:ring-0 p-0 shadow-none rounded-none font-medium text-[11px] sm:text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none"
                                  />
                                  <button
                                    className="h-full w-7 sm:w-9 flex items-center justify-center border-l hover:bg-muted transition-colors cursor-pointer"
                                    onClick={() => updateQuantity(item.productId, item.variantId, item.quantity + 1)}
                                  >
                              ...
                              <button
                                ...
                                className="text-[11px] sm:text-sm font-medium text-muted-foreground hover:text-destructive hover:underline flex items-center gap-1.5 transition-colors cursor-pointer"
                              >
                                {deletingItem?.productId === item.productId && deletingItem?.variantId === item.variantId ? <Spinner className="w-3 sm:w-3.5 h-3 sm:h-3.5" /> : "Remove"}
                              </button>
                            </div>
                          </div>
                        </div>
            ...
            {items.length > 0 && (
              <div className="bg-muted/10 border-t md:border-t-0 md:border-l p-6 md:p-8 flex flex-col justify-between shrink-0 h-auto md:h-full md:overflow-y-auto">
